...
public class PlatformSpawner : MonoBehaviour {
    ...
    void Start() {
        ...
        for(int i = 0; i < count; i++) 
        {
            ...
            platforms[i] = Instantiate(platformPrefab, poolPosition, Quaternion.identity);  // 원본, 위치, 회전
                                                                                            // Quaternion.identity = 오일러각(0,0,0)
        }
    ...
    void Update() {
        ...
        if(Time.time >= lastSpawnTime + timeBetSpawn)               // Time.time = 게임 시작 후 시간이 몇 초 지났는지 출력하는 변수 = "현재 시점"
        {
